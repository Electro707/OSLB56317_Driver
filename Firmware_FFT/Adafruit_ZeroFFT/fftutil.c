...
int ZeroFFTRealToComplex(q15_t *source, q15_t *output, uint16_t length, uint16_t do_window) {
...
int ZeroFFTMagnitude(q15_t *source, uint16_t length, uint16_t do_window) {
  int result = ZeroFFT_base(source, length, do_window);
  if (result)
      return result;
